![EEG CUDA 运算](images/the_banner.png)
# EEG_CUDA：高性能脑电信号处理加速器

[ English | [中文](./README_zh.md) ]

EEG_CUDA 是一个专为脑电图（EEG）信号处理设计的高性能库。通过利用 NVIDIA CUDA 技术，本项目在保持高数值稳定性的同时，加速了长序列、多通道数据的关键算法。

数学原理详见 `docs/math.md`。

---
## 📊 性能基准测试

我们使用以下配置在典型 EEG 数据规模上进行了全面的基准测试：
```python
### FIR Filter
```markdown
|   batch |   channels |   time_steps |   CUDA (ms) |   PyTorch (ms) |   SciPy (ms) | vs PyTorch   | vs SciPy   |
|--------:|-----------:|-------------:|------------:|---------------:|-------------:|:-------------|:-----------|
|       1 |         32 |        10000 |        0.3  |           0.55 |        12.43 | 1.8x         | 41.7x      |
|       1 |         64 |        50000 |        2.84 |           4.84 |       116.87 | 1.7x         | 41.2x      |
|       4 |         64 |        20000 |        3.61 |           5.94 |        46.96 | 1.6x         | 13.0x      |
|       8 |        128 |        10000 |        7.4  |          11.88 |        46.63 | 1.6x         | 6.3x       |
|      16 |         64 |        50000 |       37.45 |          59.65 |       114.92 | 1.6x         | 3.1x       |

### Centering
```markdown
|   batch |   channels |   time_steps |   CUDA (ms) |   PyTorch (ms) |   NumPy (ms) | vs PyTorch   | vs NumPy   |
|--------:|-----------:|-------------:|------------:|---------------:|-------------:|:-------------|:-----------|
|       1 |         32 |        10000 |        0.02 |           0.04 |         0.41 | 2.1x         | 21.4x      |
|       1 |         64 |        50000 |        0.21 |           0.22 |         5.82 | 1.1x         | 28.2x      |
|       4 |         64 |        20000 |        0.32 |           0.34 |         2.64 | 1.1x         | 8.3x       |
|       8 |        128 |        10000 |        0.45 |           0.68 |         2.92 | 1.5x         | 6.5x       |
|      16 |         64 |        50000 |        3.19 |           3.3  |         5.96 | 1.0x         | 1.9x       |

### Whitening
```markdown
|   batch |   channels |   time_steps |   CUDA (ms) |   PyTorch (ms) |   NumPy (ms) | vs PyTorch   | vs NumPy   |
|--------:|-----------:|-------------:|------------:|---------------:|-------------:|:-------------|:-----------|
|       1 |         32 |        10000 |        1.04 |           1.19 |         1.6  | 1.1x         | 1.5x       |
|       1 |         64 |        50000 |        1.75 |           1.86 |        11.53 | 1.1x         | 6.6x       |
|       4 |         64 |        20000 |        8.21 |           7.07 |         6.61 | 0.9x         | 0.8x       |
|       8 |        128 |        10000 |       18.67 |          18.85 |        11.2  | 1.0x         | 0.6x       |
|      16 |         64 |        50000 |       24.69 |          24.92 |        11.44 | 1.0x         | 0.5x       |

### FastICA
```markdown
|   batch |   channels |   time_steps |   CUDA (ms) |   sklearn (ms) | vs sklearn   |
|--------:|-----------:|-------------:|------------:|---------------:|:-------------|
|       1 |         32 |        10000 |      279.02 |         647.97 | 2.3x         |
|       1 |         64 |        50000 |      521.46 |        4178.58 | 8.0x         |
|       4 |         64 |        20000 |     1745.98 |        1933.9  | 1.1x         |
|       8 |        128 |        10000 |     4676.83 |        4372.91 | 0.9x         |
|      16 |         64 |        50000 |     7782.68 |        4311.99 | 0.6x         |

```

## 数值验证

### EEG CUDA 白化算子对齐验证报告
数据维度：Batch 10 × 62 通道 × 111000 时间点
CUDA 输出精度：torch.float32

[验证 A] 协方差最大绝对偏差：5.03196036e-06
[验证 A] 协方差平均绝对偏差：4.51245319e-07
[验证 A] 协方差最大相对偏差：1.13689825e-01

[验证 B] 数据点最大绝对误差：9.59396362e-04
[验证 B] 数据点平均绝对误差：1.41664771e-06
[验证 B] 数据点最大相对误差：7.35398026e+01

[额外检查] CUDA → 单位矩阵最大偏差：1.81416576e+00
[额外检查] PyTorch → 单位矩阵最大偏差：1.81416596e+00


### EEG CUDA FastICA 验证报告
数据维度：62 通道 × 111000 时间点
CUDA 输出精度：torch.float32

- [验证 A] 平均最大相关性：0.900836
- [验证 A] 最小相关性：0.499077
- [验证 A] 高匹配分量：40 / 62

结论：CUDA FastICA 与 scikit-learn 基线在数值上保持一致
*注：由于硬件相关的浮点截断和随机初始化，微小差异属于预期现象。*

---

## 环境要求

- Python 3.11
- PyTorch 2.6.0+cu124（需 CUDA 12.4 支持）
- CUDA Toolkit 12.4（与 PyTorch 编译版本匹配）
- 计算能力 ≥ 7.0 的 NVIDIA GPU（推荐）

本扩展基于以下环境构建并测试：
- PyTorch 2.6.0+cu124
- 构建命令：`pip install -e . --no-build-isolation`

## 安装指南

### 从源码安装（推荐以获取最新功能）

```bash
git clone https://github.com/643591947/eeg_cuda_ops.git
cd eeg_cuda_ops

# 以可编辑模式安装（推荐用于开发）
pip install -e . --no-build-isolation
